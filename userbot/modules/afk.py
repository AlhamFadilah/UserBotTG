...
async def mention_afk(mention):
    ...
    if (
        ...
        if mention.sender_id not in USERS:
            if AFKREASON:
                await mention.reply("Pengguna Telah Offline."
                                    f"\nKarena `{AFKREASON}`."
                                    f"\nAFK Sejak: {afk_str}")
            else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply("Pengguna Sedang Offline."
                                       f"\nKarena: `{AFKREASON}`."
                                       f"\nAFK Sejak: {afk_str}")
                else:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply("Dibilang lagi offline!!!."
                                           f"\nKarena: `{AFKREASON}`."
                                           f"\nAFK from: {afk_str}")
                    else:
                        await sender.reply(str(choice(AFKSTR)))
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit("Pengguna Offline!"
                         f"\nReason: `{string}`")
    else:
        await afk_e.edit("Pengguna Offline!")
    if BOTLOG:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("Pengguna Online Kembali.")
        await sleep(2)
        await msg.delete()
        if BOTLOG:
            await notafk.client.send_message(
                ...
                str(len(USERS)) + " Ketika sedang Offline",
            )
            for i in USERS:
